...
class CustomerInvoicesPaymentsReport(models.TransientModel, ReportXlsxMixin):
    _name = "customer.invoices.payments.report"
    _description = "Customer Invoices & Payments Report"
    ...
    def action_print(self):
        ...
        return self.env.ref("ft_backend.action_report_customer_invoices_payments").report_action(
            self, data=data
        )

    def check_report_xlsx(self):
        form = self._form_data()
        report_model = self.env["report.ft_backend.report_customer_invoices_payments"]
        ...
        return self._xlsx_action(
            "customer.invoices.payments.report",
    ...
    def get_xlsx_report(self, data, response):
            ...
            xlsx_rows.append(
                {
                    "values": [
                        ...
                        row.get("date", ""),
                        row.get("due_date", ""),
                        float(row.get("invoice", 0.0)),
                        float(row.get("receipt", 0.0)),
        ...
        xlsx_rows.append(
            {
                ...
                "values": [
                    ...
                    "",
                    "",
                    float(grand.get("invoice", 0.0)),
                    float(grand.get("receipt", 0.0)),
        ...
        table = {
            "sheet_name": "Customer Inv & Pay",
            "title": f"{data.get('company_name', '')}: Customer Invoices & Payments",
