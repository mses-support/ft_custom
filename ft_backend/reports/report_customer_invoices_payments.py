...
class ReportCustomerInvoicesPayments(models.AbstractModel):
    _name = "report.ft_backend.report_customer_invoices_payments"
    _description = "Customer Invoices & Payments"
    ...
    def _allocated_amount_from_line(self, line):
        ...
        for partial in partials:
            ...
            if counterpart.move_id.move_type in ("out_invoice", "out_refund"):
            ...
            rows.append(
                {
                    ...
                    "trans_type": "Invoice" if move.move_type == "out_invoice" else "Credit Note",
                    ...
                    "invoice": float(move.amount_total_signed or 0.0),
                    "receipt": 0.0,
                    "allocated": 0.0,
                    "balance": float(move.amount_residual_signed or 0.0),
    ...
    def _payment_rows(self, options):
        ...
        for move in moves:
            receivable_lines = move.line_ids.filtered(
                lambda line: line.account_id.account_type == "asset_receivable" and line.partner_id
            )
            for line in receivable_lines:
                if partner_filter and line.partner_id.id not in partner_filter:
                    continue
                receipt = float(-line.balance or 0.0)
                allocated = self._allocated_amount_from_line(line)
                if not receipt and not allocated:
                    continue
                rows.append(
                    {
                        "row_type": "line",
                        ...
                        "trans_type": "Receipt" if receipt >= 0 else "Payment",
    ...
    def _build_rows(self, options):
        ...
        result_rows = []
        grand = {"invoice": 0.0, "receipt": 0.0, "allocated": 0.0, "balance": 0.0}
        ...
        for partner in partners:
            ...
            for row in partner_lines:
                ...
                grand["invoice"] += row["invoice"]
                grand["receipt"] += row["receipt"]
