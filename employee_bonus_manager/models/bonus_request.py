...
class BonusRequest(models.Model):
    ...
    def action_post_journal_entry(self):
        ...
        if self.state != 'manager_approved':
            raise UserError('Only manager approved requests can be sent to accounting.')
        if not self.journal_id:
            raise UserError('Please set a bonus journal before posting the accounting entry.')
        if not self.credit_account_id or not self.debit_account_id:
            raise UserError('Please set both debit and credit accounts before posting.')
        if self.bonus_amount <= 0:
            raise UserError('Bonus amount must be greater than zero.')
        account_move = self.env['account.move'].create({
            'ref': self.reference,
            'date': self.manager_approved_date or fields.Date.today(),
