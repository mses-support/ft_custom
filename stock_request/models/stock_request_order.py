...
class StockRequestOrder(models.Model):
    ...
    reference_id = fields.Many2one(
        ...
        "reference. If none is given, a dedicated reference will be created.",
    )
    ...
    def action_submit(self):
        for order in self:
            if not order.stock_request_ids:
                raise UserError(
                    _(
                        "There should be at least one request line before submitting."
                    )
                )
        self.write({"approval_state": "submitted"})
        return True

    def _prepare_internal_picking_vals(self):
        self.ensure_one()
    ...
    def _prepare_internal_move_vals(self, line, picking):
        vals = {
        ...
        if self.reference_id:
            vals["reference_ids"] = [(6, 0, self.reference_id.ids)]
        return vals

    def _create_internal_transfer(self):
        StockMove = self.env["stock.move"].sudo()
        Allocation = self.env["stock.request.allocation"].sudo()
        created_pickings = self.env["stock.picking"]
        for order in self:
            active_pickings = order.picking_ids.filtered(lambda p: p.state != "cancel")
            if active_pickings:
                continue
            if not order.stock_request_ids:
                continue
            if order.source_location_id == order.location_id:
                raise UserError(
                    _("Source and destination locations must be different.")
                )
            picking = self.env["stock.picking"].sudo().create(
                order._prepare_internal_picking_vals()
            )
            for line in order.stock_request_ids:
                move = StockMove.create(order._prepare_internal_move_vals(line, picking))
                Allocation.create(
                    {
            ...
            order.stock_request_ids.filtered(lambda r: r.state == "draft").write(
                {"state": "open"}
            )
    ...
    def action_approve(self):
        for order in self:
            if order.approval_state not in ("submitted", "draft"):
                continue
            order._create_internal_transfer()
        self.write(
            {
    ...
    def action_draft(self):
        self.mapped("stock_request_ids").filtered(lambda r: r.state == "cancel").action_draft()
        self.write(
            {"approval_state": "draft", "approved_by": False, "approved_date": False}
        )
        return True
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if not upd_vals.get("source_location_id") and upd_vals.get("warehouse_id"):
                warehouse = self.env["stock.warehouse"].browse(upd_vals["warehouse_id"])
                upd_vals["source_location_id"] = warehouse.lot_stock_id.id
            vals_list_upd.append(upd_vals)
        return super().create(vals_list_upd)

    def unlink(self):
        if self.filtered(lambda r: r.approval_state != "draft"):
    ...
    @api.constrains("source_location_id", "company_id")
    def _check_source_location_company(self):
        if any(
            request.source_location_id.company_id
            and request.source_location_id.company_id != request.company_id
            for request in self
        ):
            raise ValidationError(
                _(
                    "The company of the stock request must match with "
                    "that of the source location."
                )
            )
