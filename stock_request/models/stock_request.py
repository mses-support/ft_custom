...
class StockRequest(models.Model):
    ...
    @api.constrains("order_id", "reference_id")
    def check_order_reference(self):
        for rec in self:
            if rec.order_id and rec.order_id.reference_id != rec.reference_id:
                raise ValidationError(_("Reference must be equal to the order"))
    ...
    def _prepare_procurement_values(self, reference=False):
        ...
        """
        self.ensure_one()
        reference = reference or self.reference_id
        if not reference:
    ...
    def _action_launch_procurement_rule(self):
        ...
        for request in self:
            ...
            values = request._prepare_procurement_values(reference=request.reference_id)
            try:
                procurements = []
                procurements.append(
