...
class HrOvertime(models.Model):
    ...
    contract_id = fields.Many2one('hr.version', string="Contract",
                                  related="employee_id.current_version_id",
                                  help="Current contract version of the employee")
    ...
    def _get_project_manager(self):
        ...
        for sheet in self:
            if sheet.project_id:
                sheet.project_manager_id = sheet.project_id.user_id.id
            else:
                sheet.project_manager_id = False
    ...
    def create(self, vals_list):
        """Create overtime requests with unique sequence numbers."""
                ...
                for over_time in overtime_dates:
                    for leave_date in leave_dates:
                        if leave_date == over_time:
                            holiday = True
            if holiday:
                self.public_holiday = 'You have Public Holidays in your Overtime request.'
            else:
                self.public_holiday = ' '
